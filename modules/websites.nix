...
let
  ...
  ryanWebsite = mkStaticWebsite {
    ...
    extraConfig = ''
      ...
      add_header X-Frame-Options SAMEORIGIN always;
      add_header X-Content-Type-Options nosniff always;
      add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; frame-src 'self' https://watch.eeg.cl.cam.ac.uk;" always;
      add_header Referrer-Policy 'same-origin';
    '';
  };
