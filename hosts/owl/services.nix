  ...
  services.nginx.virtualHosts."meands.org" = {
    ...
    extraConfig = ''
      add_header Strict-Transport-Security max-age=31536000 always;
        ...
        add_header X-Content-Type-Options nosniff always;
        add_header Referrer-Policy 'same-origin';
        add_header Content-Security-Policy "default-src 'self' 'unsafe-inline' 'unsafe-eval' blob:; base-uri 'self'; frame-src 'self'; frame-ancestors 'self'; form-action 'self'; style-src 'self' https://cdn.jsdelivr.net; font-src 'self' https://fonts.gstatic.com; script-src 'self' https://cdn.jsdelivr.net;" always;
    '';
  };
